...
def find_or_create_path(fs, path, is_dir=False, real_size=0):
    ...
    for i, part in enumerate(parts):
        ...
        if not found:
            ...
            # Set correct Linux Mode bits (S_IFDIR for dirs, S_IFREG for files)
            mode = (stat.S_IFDIR | 0o755) if actual_is_dir else (stat.S_IFREG | 0o644)
            size = 4096 if actual_is_dir else real_size
            contents = [] if actual_is_dir else b""
...
def sync():
    ...
    fs = load_fs()
    print(f"--- Syncing HoneyFS from: {HONEYFS_DIR} ---")
    ...
    for root, dirs, files in os.walk(HONEYFS_DIR):
        ...
        for d in dirs:
            find_or_create_path(fs, os.path.join(rel_root, d), is_dir=True)
